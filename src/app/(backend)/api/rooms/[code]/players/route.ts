...
import { roomCodeSchema, roomHostPlayerSchema } from "@/shared/schemas";
...
export async function POST(
  ...
  try {
    ...
    const result = await withSerializableRetry(async () =>
      prisma.$transaction(
        async (tx) => {
          ...
          const createResult = await addPlayerToRoom(
            tx,
            access.room,
            playerName,
            true,
    ...
    if (result.kind !== "created") {
      return jsonError("ไม่สามารถเพิ่มผู้เล่นได้ในขณะนี้", 500);
    }
